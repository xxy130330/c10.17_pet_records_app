...
class PetList extends Component {
            ...
            <div className="card petListCard">
              ...
              <div className="card-block">
                <p className="card-text">
                  The place where you can share your pet's medical information
  ...
  deleteFromServer() {
    ...
    this.props
      .delete_pet(petDataProps[petIndex]["ID"])
      .then(() =>
        this.props
          .fetchPetData(localStorage.getItem("id"))
          .then(() =>
            this.setState({ ...this.state, showModal: false, canDelete: false })
          )
      );
  ...
  render() {
    ...
    const userPetList = this.props.petdata.map((item, index) => {
      ...
      const item_name =
        item.name.length > 8 ? (
          <h4 className="petListName">{item.name}</h4>
        ) : (
          <h2 className="petListName">{item.name}</h2>
        );
      return (
        <div key={index} className="row justify-content-center petRow">
          <Link
            className=""
            to={"/pet-profile/" + this.props.petdata[index]["ID"]}
          >
            <div className="petAvatar" style={petAvatar} />
          </Link>
          <div className="nameContainer">{item_name}</div>
          <div className="deleteButton">
            <i
              onClick={() => {
                this.triggerModal(index);
              }}
              className={
                this.state.canDelete
                  ? "fa fa-times-circle aria-hidden=true fa-3x "
                  : ""
              }
            />
          </div>
    ...
    return (
      <div className="bodyContainer">
        <div className="petListContainer">
          ...
          <div className="container">{userPetList}</div>
        </div>
        <div className="deleteNAddContainer">
          <button
            ...
            onClick={() => {
              this.setState({ canDelete: toggleCanDelete });
            }}
          >
            {!this.state.canDelete ? "Delete a Pet" : "Cancel"}
          </button>
          <Link to="/add-pet/">
            <button className="btn btn-outline-success addPet">
              Add a Pet
            </button>
          </Link>
